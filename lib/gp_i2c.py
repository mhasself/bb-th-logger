...
def export_gpio(n):
    if os.path.exists(GPIO_BASE + 'gpio%i' % n):
        return
    print 'Exporting %i' % n
    f = open(GPIO_BASE + 'export', 'w')
    f.write('%i\n' % n)
    f.close()
...
class Pin:
    ...
    def __init__(self, pin_num):
        if isinstance(pin_num, basestring):
            import pins
            pin_num = pins.get_pin(pin_num)['gpio']
        export_gpio(pin_num)
        self.filename = '%sgpio%i/direction' % (GPIO_BASE, pin_num)
        self.vilename = '%sgpio%i/value' % (GPIO_BASE, pin_num)
        self.set_hi()
...
class I2CGang:
    ...
    def __init__(self, clk, dats):
        self.clk = Pin(clk)
        self.dats = [Pin(d) for d in dats]
        self.delay_s = 1e-4
    def wait(self, n=1):
